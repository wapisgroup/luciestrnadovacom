...
function _isTablet() {
  if (REG_TABLETS.IPAD.test(_userAgent) || REG_TABLETS.KINDLE.test(_userAgent) || REG_TABLETS.PLAYBOOK[0].test(_userAgent) || REG_TABLETS.PLAYBOOK[1].test(_userAgent) || REG_TABLETS.TABLET.test(_userAgent)) {
    return true
  } else {
    return false;
  }
...
function _isMobile() {
  if (REG_MOBILES.GENERIC_REG[0].test(_userAgent) && _isTablet() == false ||
    REG_MOBILES.GENERIC_REG[1].test(_userAgent.substr(0, 4)) && _isTablet() == false) {
    return true
  } else {
    return false;
  }
...
function _isDesktop() {
  return (_isMobile() !== false && _isTablet() !== false);
}

export default {_isTablet, _isMobile, _isDesktop};
